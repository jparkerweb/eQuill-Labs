...
- [x] **Task 2.2:** Install pipeline deps: `npm install @octokit/graphql unified remark-parse remark-gfm mdast-util-to-string gray-matter`. Install types: `npm install -D @types/node`. Verify all land in `dependencies`/`devDependencies` correctly.
...
- **Implementer:** Claude Opus 4.6 (AI-assisted)
- **Commits:** _(pending user commit)_
- **What was done:** Built the pure-function Node content pipeline: Zod content collection schemas (`src/content/config.ts`), pipeline utilities (`scripts/lib/{sort-keys,hash,graphql-queries,match-pattern,change-set}.ts`), GitHub fetcher via `@octokit/graphql` + `gh auth token` (`scripts/fetch-github.ts`), AST-based README parser (`src/lib/readme-parse.ts`), curation module with glob-pattern exclusion (`scripts/curate.ts`), content renderer with schema validation + atomic writes + skip-write idempotency (`scripts/render-content.ts`), build manifest writer (`scripts/build-manifest.ts`), initial curation overrides (`site/featured.json`) + JSON Schema (`site/featured.schema.json`), `data/` scaffold, and npm scripts `refresh:fetch` / `refresh:render` / `refresh:all`. Smoke run fetched 59 repos, rendered 54 project `.md` files (5 hidden via `site/featured.json`), with 6 featured projects. `astro check` passes with 0 errors/0 warnings. Second `npm run refresh:all` run is byte-idempotent (0 changes).
- **Files changed:**
  - Added: `src/content/config.ts`, `src/lib/readme-parse.ts`
  - Added: `scripts/fetch-github.ts`, `scripts/curate.ts`, `scripts/render-content.ts`, `scripts/build-manifest.ts`
  - Added: `scripts/lib/sort-keys.ts`, `scripts/lib/hash.ts`, `scripts/lib/graphql-queries.ts`, `scripts/lib/match-pattern.ts`, `scripts/lib/change-set.ts`
  - Added: `site/featured.json`, `site/featured.schema.json`
  - Added: `data/github-snapshot.json`, `data/ai-cache.json`, `data/build-manifest.json`, `data/proposals/.gitkeep`
  - Added: 54 × `src/content/projects/<slug>.md`
  - Modified: `package.json` (refresh:* scripts + deps), `package-lock.json`
- **Issues encountered:**
  - Astro 5 content collections reserve `slug` as a routing field — extracted from frontmatter before Zod validation. The Zod schema cannot require `slug`. Resolution: removed `slug` from Zod in `src/content/config.ts` + `scripts/render-content.ts`; `slug` still written to frontmatter (Astro uses it as canonical slug); `id` mirrors slug.
  - Task 2.6(g) writes `fetchedAt: new Date().toISOString()` on every run, which would propagate to `_source.fetchedAt` in every `.md` file and break Task 2.15(d)'s idempotency contract. Resolution: `fetch-github.ts` compares `{profile, repos}` against the existing snapshot; if identical, preserves the existing `fetchedAt` (skip-write on content, updates-only on real change). This keeps `refresh:all` byte-idempotent.
- **Deviations from plan:** See above (both documented inline as SPEC NOTEs during implementation).
